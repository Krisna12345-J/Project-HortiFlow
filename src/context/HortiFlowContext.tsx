...
export const HortiFlowProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [users, setUsers] = useState<User[]>(() => {
    const saved = localStorage.getItem('hortiflow_users');
  ...
  const getAssetsForPackage = useCallback(
    (packageId: string) => assetsRef.current.filter((a) => a.packageId === packageId),
    []
  );
  ...
  const reviewFindings = useMemo<ReviewFinding[]>(() => {
    return (Object.values(reviews) as ReviewFinding[][]).flat();
  }, [reviews]);

  const approvalManifests = useMemo<ApprovalManifest[]>(() => {
    return Object.values(manifests) as ApprovalManifest[];
  }, [manifests]);

  const publicationRecords = useMemo<PublicationPlan[]>(() => {
    return (Object.values(publicationPlans) as PublicationPlan[][]).flat();
  }, [publicationPlans]);
